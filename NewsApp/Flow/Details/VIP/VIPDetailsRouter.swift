...
protocol VIPArticleDetailsRouterProtocol: AnyObject {
    func presentShareScreen(with url: String)
    func presentArticle(at url: String)
}

class VIPArticleDetailsRouter: Coordinator {
    ...
    func start() {
        ...
        let controller = VIPDetailConfigurator.createDetailsModule(with: self, homeViewModel: homeViewModel, article: article)
        navigationController.pushViewController(controller, animated: true)
    }
}

extension VIPArticleDetailsRouter: VIPArticleDetailsRouterProtocol {
    func presentShareScreen(with url: String) {
